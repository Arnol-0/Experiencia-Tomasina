...
const Dashboard = () => {
  ...
  return (
    <div className="dashboard-page container">
      ...
      {showSystemUsersModal && isGlobal && (
        <div className="dashboard-modal-overlay">
          <div className="dashboard-modal-content animate-fade-in" style={{ maxWidth: '800px' }}>
            <button className="dashboard-modal-close" onClick={() => setShowSystemUsersModal(false)}>
              <X size={24} />
            </button>
            <div className="section-header">
              ...
              {staffList.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
                  {staffList.map(staffUser => (
                    <div key={staffUser.id} className="staff-item" style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                      {editingSystemUserId === staffUser.id ? (
                        <div style={{ display: 'flex', gap: '1rem', flex: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                          <input 
                            type="text" 
                            value={editSystemUserForm.name}
                            onChange={(e) => setEditSystemUserForm({ ...editSystemUserForm, name: e.target.value })}
                          ...
                          <CustomSelect 
                            ...
                            placeholder="Sede"
                          />
                          <button onClick={() => handleSaveSystemUser(staffUser.id)} className="btn-outline-green" style={{ padding: '0.5rem' }} title="Guardar">
                            <Check size={18} />
                          </button>
                          <button onClick={() => setEditingSystemUserId(null)} className="btn-outline-red" style={{ padding: '0.5rem' }} title="Cancelar">
                            <X size={18} />
                          </button>
                        </div>
                      ) : (
                        <>
                          <div style={{ display: 'flex', flexDirection: 'column' }}>
                            <span className="staff-name">{staffUser.name} <span style={{ fontSize: '0.75rem', fontWeight: 'normal', color: 'var(--text-muted)' }}>({staffUser.email})</span></span>
                            <span className="staff-email">Rol: {staffUser.role} | Sede: {staffUser.branch}</span>
                          </div>
                          <div style={{ display: 'flex', gap: '0.5rem' }}>
                            ...
                            <button 
                              onClick={() => handleDeleteSystemUser(staffUser.id)} 
                ...
                <p className="no-staff">No hay usuarios en el sistema.</p>
              )}
            </div>
          </div>
        </div>
